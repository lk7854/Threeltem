...
public class RecvMoThread extends Thread {
	 ...
	 */
	public void run() {
		// 上行集合 本集合临时存储上行，态��将收到的上行保存在一个队列中，由另外态��线程去处理态
		...
		while (true) {
			try {
				...
				if (result == 0 && mos != null && mos.size() > 0) {
					...
					// 添加到上行队态
					// add mo queue
					// 这里不要做复杂的耗时的处理，将收到的上行保存在一个队列中，由另外态��线程去处理态

					// 代码示例是将上行信息打印出来
					System.out.println("获取上行成功！获取到的上行有" + mos.size() + "条记录态");
					...
					// 如果获取上行失败，则将错误码打印
					if (result != 0) {
						System.out.println("获取上行失败，错误码态" + result);
					}
